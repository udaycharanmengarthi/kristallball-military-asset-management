...
import { Radar, Lock, User, Loader2, AlertTriangle } from "lucide-react";
...
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Unable to sign in. Check your connection.");
    } finally {
      setLoading(false);
    }
  }
        ...
        <div className="w-full max-w-sm">
          <div className="mb-8 flex flex-col items-center text-center">
            <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-lg border border-brass-600/40 bg-brass-500/10">
              <Radar className="h-7 w-7 text-brass-400" strokeWidth={1.5} />
            </div>
            <h1 className="font-mono text-xl font-semibold tracking-widest2 text-mist-50">
              KRISTALLBALL
            </h1>
            <p className="mt-1 text-sm text-mist-400">Military Asset Management System</p>
          </div>
          ...
          >
            <div className="mb-4">
              ...
              <div className="flex items-center gap-2 rounded-md border border-ink-600 bg-ink-800 px-3 py-2.5 focus-within:border-brass-500">
                <User className="h-4 w-4 text-mist-500" strokeWidth={1.75} />
                <input
                  autoFocus
                  value={username}
            ...
            <div className="mb-5">
              ...
              <div className="flex items-center gap-2 rounded-md border border-ink-600 bg-ink-800 px-3 py-2.5 focus-within:border-brass-500">
                <Lock className="h-4 w-4 text-mist-500" strokeWidth={1.75} />
                <input
                  type="password"
                  value={password}
          ...
          <div className="mt-5 rounded-md border border-ink-700 bg-ink-850/50 px-4 py-3 text-xs text-mist-400">
            ...
            <div className="space-y-0.5">
              <div>admin_user / AdminPass123!</div>
              <div>commander_alpha / CommandPass123! (Fort Alpha)</div>
              <div>commander_bravo / CommandPass123! (Fort Bravo)</div>
              <div>commander_charlie / CommandPass123! (Fort Charlie)</div>
              <div>logistics_officer / LogisticsPass123!</div>
            </div>
          </div>
        </div>
      </div>
